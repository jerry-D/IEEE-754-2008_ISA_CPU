...
module XCUpc (
 ...
 input         ld_vector;
 input  [`XPCWIDTH-1:0] vector;
 input         rewind_PC;
 input         wrcycl; 
 output        discont_out;       
 input  [15:0] OPsrcB_q2;
 input         RPT_not_z;
 input  [`XPCWIDTH-1:0] pre_PC;
 output [`XPCWIDTH-1:0] PC;
 output [`XPCWIDTH-1:0] PC_COPY;
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) begin
        ...
        if (ld_vector) begin
            ...
            else if (bitmatch) PC_COPY <= pc_q2 + {{`XPCWIDTH-14{OPsrcB_q2[13]}}, OPsrcB_q2[13:0]};
            else if ((OPdest_q2==JMPA_) && wrcycl && ~Ind_Dest_q2 && q2_sel) PC_COPY <= wrsrcAdata[`XPCWIDTH-1:0];  
           ...
           branchJustTaken <= 1'b1; 
            ...
            PC <= pc_q2 + {{`XPCWIDTH-14{OPsrcB_q2[13]}}, OPsrcB_q2[13:0]};             
            PC_COPY <= pc_q2 + 1'b1; 
            branchJustTaken <= 1'b1;             
        end         
        else if ((OPdest_q2==JMPA_) && q2_sel && wrcycl && ~Ind_Dest_q2) begin
            PC <= wrsrcAdata[`XPCWIDTH-1:0];                       
            PC_COPY <= pc_q2 + 1'b1;  
            branchJustTaken <= 1'b1; 
        end      
        else begin
