...
wire signed [64:0] oprndA_signed;
wire signed [64:0] oprndB_signed;
...
assign oprndA_signed[64:0] = $signed({(Sext_SrcA_q2 && oprndA[63]), oprndA[63:0]});
assign oprndB_signed[64:0] = $signed({(Sext_SrcB_q2 && oprndB[63]), oprndB[63:0]});    
  
assign SUB_result[64:0] = oprndA_signed - oprndB_signed - (C && Sext_Dest_q2);  //destination Sext bit serves as signal for SUBC vs SUB
assign z_flag = ~|SUB_result;
assign n_flag = SUB_result[64];
assign c_flag = (~oprndA[63] && oprndB[63]) || (oprndB[63] && SUB_result[63]) || (~oprndA[63] && SUB_result[63]); 
assign v_flag = (oprndA[63] && ~oprndB[63] && ~SUB_result[63]) || (~oprndA[63] && oprndB[63] && SUB_result[63]);


RAM_func #(.ADDRS_WIDTH(4), .DATA_WIDTH(68))
    ram32_integer_SUB(
    .CLK        (CLK      ),
    .wren       (wren     ),
    .wraddrs    (wraddrs  ),
