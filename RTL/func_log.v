...
input [9:0] NaN_del;
input [3:0] wraddrs_del, wraddrs, rdaddrsA, rdaddrsB;
input A_sign_del;   
input A_is_zero_del;
input A_invalid_del;
input A_inexact_del;
...
assign invalid = A_sign_del && wren_del_6 && ~A_is_zero_del; // A is a negative non-zero number (including infinity)
                   
assign div_by_0 = A_is_zero_del && wren_del_6 ;                  
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) semaphor <= 16'hFFFF;
        ...
        if (wren_del_7) semaphor[wraddrs_del] <= 1'b1;
    end
end
