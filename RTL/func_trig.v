...
reg [3:0] SIN_semaphor;
reg [3:0] COS_semaphor;
reg [3:0] TAN_semaphor;
reg [3:0] COT_semaphor;
reg [5:0] delay0;
...
assign SIN_wrenq = delay0[5];
assign COS_wrenq = delay0[4];
assign TAN_wrenq = delay0[3];
assign COT_wrenq = delay0[2];
assign wraddrsq = delay0[1:0]; 
...
RAM_func #(.ADDRS_WIDTH(2), .DATA_WIDTH(18))
   ram32_SIN(
   .CLK        (CLK      ),
   .wren       (SIN_wrenq),
   .wraddrs    (wraddrsq[1:0]),     //includes thread#
   .wrdata     (SINout510),
   .rdenA      (SIN_rdenA),
   .rdaddrsA   (rdaddrsA ),
   .rddataA    (SIN_rddataA),                                  
