...
module RAM64_byte #(parameter ADDRS_WIDTH = 12) (
    CLK,
    CEN,            //clock enable active low
    CE123,          //chip enable active high
    WE,             //active low
    BWh,            //active low
    BWg,            //active low
    BWf,            //active low
...
input BWa;
input adv_LD;
input [ADDRS_WIDTH-1:0] A;
inout [63:0] DQ;
input OE;


reg [63:0] RAM64[(2**ADDRS_WIDTH)-1:0];
/*
//integer i, r, file;
integer i;
...
assign wren = ~CEN && ~WE && CE123;

assign DQ = (~OE && ~WEq) ? rddata : 64'bz;
assign wrdata = DQ; 

assign rden = ~CEN && CE123;    
                                                                                                                                 
always @(posedge CLK) 
    if (rden && ~adv_LD) rddata <= RAM64[A];
...
always @(posedge CLK) begin 
    ...
    if (WEq) begin
        if (BWh_q) RAM64[Aq][63:56] <= wrdata[63:56];
        if (BWg_q) RAM64[Aq][55:48] <= wrdata[55:48];
        if (BWf_q) RAM64[Aq][47:40] <= wrdata[47:40];
        if (BWe_q) RAM64[Aq][39:32] <= wrdata[39:32];
        if (BWd_q) RAM64[Aq][31:24] <= wrdata[31:24];
        if (BWc_q) RAM64[Aq][23:16] <= wrdata[23:16];
        if (BWb_q) RAM64[Aq][15: 8] <= wrdata[15: 8];
        if (BWa_q) RAM64[Aq][ 7: 0] <= wrdata[ 7: 0];
    end
end
