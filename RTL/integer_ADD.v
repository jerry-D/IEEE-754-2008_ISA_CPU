...
assign oprndA_signed[64:0] = $signed({(Sext_SrcA_q2 && oprndA[63:0]), oprndA[63:0]});
assign oprndB_signed[64:0] = $signed({(Sext_SrcB_q2 && oprndB[63:0]), oprndB[63:0]});    
  
assign ADD_result[64:0] = oprndA_signed + oprndB_signed + (C && Sext_Dest_q2);  //destination Sext bit serves as signal for ADDC vs ADD
assign z_flag = ~|ADD_result;
assign n_flag = ADD_result[64];
assign c_flag = (~oprndA[63] && oprndB[63]) || (oprndB[63] && ADD_result[63]) || (~oprndA[63] && ADD_result[63]); 
assign v_flag = (oprndA[63] && ~oprndB[63] && ~ADD_result[63]) || (~oprndA[63] && oprndB[63] && ADD_result[63]);
