...
reg [`RPTSIZE-1:0] REPEAT_int;

wire RPT_not_z_a;
assign RPT_not_z_a = |REPEAT_a; 

wire RPT_not_z_int;
assign RPT_not_z_int = |REPEAT_int; 

wire auto_post_modify_instr_a;
assign auto_post_modify_instr_a = (~Dam_q0[1] || ~Dam_q0[0]) && RPT_not_z_a &&
                              ((Ind_Dest_q0 && ~Imod_Dest_q0) ||                                                        
                               (Ind_SrcA_q0 && ~Imod_SrcA_q0) ||                                                       
                               (Ind_SrcB_q0 && ~Imod_SrcB_q0));                                                        
wire auto_post_modify_instr_int;
assign auto_post_modify_instr_int = (~Dam_q0[1] || ~Dam_q0[0]) && RPT_not_z_int &&
                              ((Ind_Dest_q0 && ~Imod_Dest_q0) ||                                                        
                               (Ind_SrcA_q0 && ~Imod_SrcA_q0) ||                                                       
                               (Ind_SrcB_q0 && ~Imod_SrcB_q0));                                                        
...
assign REPEAT = int_in_service ? REPEAT_int : REPEAT_a;                                                                                      

wire RPT_not_z;
assign RPT_not_z = int_in_service ? RPT_not_z_int : RPT_not_z_a; 
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) begin
        ...
        if ( (Dam_q0[0] && (OPdest_q0==REPEAT_addrs)) && ~Ind_Dest_q0 && ~int_in_service && ~discont) REPEAT_a <= OPsrcB_q0[`RPTSIZE-1:0]; //load REPEAT reg with 11-bit immediate during q0
        else if ((~Dam_q0[1] && (OPdest_q0==REPEAT_addrs)) && Ind_SrcA_q0 && ~Ind_Dest_q0 && ~int_in_service && ~discont) begin
            casex(ARn_sel_q0)
                3'b000 : REPEAT_a <= AR0[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b001 : REPEAT_a <= AR1[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b010 : REPEAT_a <= AR2[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b011 : REPEAT_a <= AR3[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b100 : REPEAT_a <= AR4[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b101 : REPEAT_a <= AR5[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b110 : REPEAT_a <= AR6[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                  default : REPEAT_a <= 0;
            endcase
        end
        else if (auto_post_modify_instr_a && ~break_q0 && ~int_in_service) REPEAT_a <= REPEAT_a - 1'b1;  
    end
end        

//for use within interrupt service routine
always @(posedge CLK or posedge RESET) begin
    if (RESET) begin
        REPEAT_int <= 0;
    end
    else begin
        if ( (Dam_q0[0] && (OPdest_q0==REPEAT_addrs)) && ~Ind_Dest_q0 && int_in_service && ~discont) REPEAT_int <= OPsrcB_q0[`RPTSIZE-1:0]; //load REPEAT reg with 11-bit immediate during q0
        else if ((~Dam_q0[1] && (OPdest_q0==REPEAT_addrs)) && Ind_SrcA_q0 && ~Ind_Dest_q0 && int_in_service && ~discont) begin
            casex(ARn_sel_q0)
                3'b000 : REPEAT_int <= AR0[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b001 : REPEAT_int <= AR1[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b010 : REPEAT_int <= AR2[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b011 : REPEAT_int <= AR3[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b100 : REPEAT_int <= AR4[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b101 : REPEAT_int <= AR5[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                3'b110 : REPEAT_int <= AR6[`RPTSIZE-1:0];  //load REPEAT reg with contents of specified ARn
                  default : REPEAT_int <= 0;
            endcase
        end
        else if (auto_post_modify_instr_int && ~break_q0 && int_in_service) REPEAT_int <= REPEAT_int - 1'b1;  
    end
end        
